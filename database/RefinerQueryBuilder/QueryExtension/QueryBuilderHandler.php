<?php namespace RefinerQueryBuilder\QueryExtension;
...
class QueryBuilderHandler
{
    ...
    /**
     * @var \RefinerQueryBuilder\QueryExtension\Adapters\BaseAdapter
     */
    ...
    /**
     * @param null|\RefinerQueryBuilder\Connection $connection
     *
     * @param int $fetchMode
    ...
    public function __construct(Connection $connection = null, $fetchMode = PDO::FETCH_OBJ)
    {
        if (is_null($connection)) {
            if (!$connection = Connection::getStoredConnection()) {
                throw new RefinerQueryBuilderException('No database connection found.', 1);
            }
        }

        $this->connection = $connection;
        $this->pack = $this->connection->getContainer();
        ...
        $this->adapterInstance = $this->pack->build(
            '\\RefinerQueryBuilder\\QueryExtension\\Adapters\\' . ucfirst($this->adapter),
   ...
   * Get first row
   * @return mixed|null
   * @throws \RefinerQueryBuilder\RefinerQueryBuilderException
   */
  ...
  protected function aggregate($type)
    ...
    public function getQuery($type = 'select', $dataToBePassed = array())
    {
        ...
        if (!in_array(strtolower($type), $allowedTypes)) {
            throw new RefinerQueryBuilderException($type . ' is not a known type.', 2);
        }

        $queryArr = $this->adapterInstance->$type($this->statements, $dataToBePassed);

        return $this->pack->build(
            '\RefinerQueryBuilder\QueryExtension\QueryObject',
  ...
  /**
   * @param \RefinerQueryBuilder\QueryExtension\QueryBuilderHandler $queryBuilder
   * @param null $alias
   * @return mixed
   * @throws \RefinerQueryBuilder\RefinerQueryBuilderException
   */
  ...
  /**
   * @param $data
   * @return array|mixed|\RefinerQueryBuilder\QueryExtension\QueryBuilderHandler|string|null
   * @throws \RefinerQueryBuilder\RefinerQueryBuilderException
   */
  ...
  public function join($table, $key, $operator = null, $value = null, $type = 'inner')
    {
        ...
        $joinBuilder = $this->pack->build('\\RefinerQueryBuilder\\RefinerQueryBuilder\\JoinBuilder', array($this->connection));
    ...
    public function transaction(\Closure $callback)
    {
        try {
            ...
            $transaction = $this->pack->build('\\RefinerQueryBuilder\\RefinerQueryBuilder\\Transaction', array($this->connection));
   ...
   * @param null $operator
   * @param null $value
   * @return \RefinerQueryBuilder\QueryExtension\QueryBuilderHandler
   * @throws \Exception
   */
  ...
  public function raw($value, $bindings = array())
    {
        return $this->pack->build('\\RefinerQueryBuilder\\QueryExtension\\Raw', array($value, $bindings));
    }
